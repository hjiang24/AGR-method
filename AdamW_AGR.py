...
class AdamW(Optimizer):
    ...
    def _init_group(
        ...
        for p in group["params"]:
            ...
            # Embed the AGR method 
            ###
            grad = p.grad
            coefficient = torch.abs(grad)/torch.abs(grad).sum(dim=tuple(range(0,len(grad.size()))),keepdim=True)
            p.grad = grad - torch.mul(grad,coefficient)
            ###
            grads.append(p.grad)
            state = self.state[p]
    ...
    def step(self, closure=None):
        ...
        return loss # by author
        # return dic_coefficient # by self
