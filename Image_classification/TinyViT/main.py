...
def main(args, config):
    ...
    #     if parameters.requires_grad and len(list(parameters.size()))>3:
    #         if parameters.size not in size:
    #             size.append(parameters.data.size())
    # print(len(size))
    ...
    kernel_128 = torch.ones(128, 1, 3, 3).to(device)
    kernel_160 = torch.ones(160, 1, 3, 3).to(device)
    kernel_320 = torch.ones(320, 1, 3, 3).to(device)
    dic_coefficient = {kernel_128.size():0, kernel_160.size():0, kernel_320.size():0}
    list_kernel = [kernel_128,kernel_160,kernel_320]
    for key in list_kernel:
        sum = key.sum(dim=tuple(range(1,len(key.size()))),keepdim=True)
        coefficient = key/sum
        dic_coefficient[key.size()] = coefficient
    ...
    if not args.only_cpu:
        model = torch.nn.parallel.DistributedDataParallel(  # by author
            model, device_ids=[config.LOCAL_RANK], broadcast_buffers=False)
        model_without_ddp = model.module
    else:
        model_without_ddp = model

    # loss_scaler = NativeScalerWithGradNormCount(grad_scaler_enabled=config.hasattr)  # by author
    loss_scaler = NativeScalerWithGradNormCount(grad_scaler_enabled=True)  # by self

    # n_parameters = sum(p.numel()
    #                    for p in model.parameters() if p.requires_grad)
    # logger.info(f"number of params: {n_parameters}")   # by author
    ...
    dic_record = {"loss_train":[],"accuracy_train":[],"loss_val":[],"accuracy_val":[]}
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if config.DISTILL.ENABLED:
            ...
            loss_train, acc_train = train_one_epoch(args, config, model, criterion,
                            data_loader_train, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler)
            dic_record["loss_train"].append(loss_train)
            dic_record["accuracy_train"].append(acc_train)
        ...
        acc1, acc5, loss = validate(args, config, data_loader_val, model)
        dic_record["loss_val"].append(loss)
        dic_record["accuracy_val"].append(acc1) 
        logger.info(
            f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
    ...
    with open("dic_record.pkl","wb") as file:
        pickle.dump(dic_record,file)
...
if __name__ == '__main__':
    ...
    if args.only_cpu:
        ...
        torch.cuda.set_device(config.LOCAL_RANK) # by author
        ddp_backend = 'nccl'
