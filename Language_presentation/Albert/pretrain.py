...
def evaluate_sop(model, batch):
    input_ids, segment_ids, input_mask, masked_ids, masked_pos, masked_weights, is_next = batch
    _, logits_clsf = model(input_ids, segment_ids, input_mask, masked_pos)
    target = logits_clsf.softmax(dim=1)
    _, pred_next = target.max(1)
    result = (pred_next == is_next).float() #.cpu().numpy()
    accuracy = result.mean().item()
    return accuracy*100

# def evaluate_mlm(model, batch):
#     input_ids, segment_ids, input_mask, masked_ids, masked_pos, masked_weights, is_next = batch
#     logits_lm, logits_clsf = model(input_ids, segment_ids, input_mask, masked_pos)
#     target = logits_lm.softmax(dim=-1)  # for masked LM
#     _, pred_masked = target.transpose(1,2).max(1)
#     print(pred_masked.size(), masked_ids.size())
#     accuracy = (pred_masked == masked_ids).float()
#     accuracy = (accuracy*masked_weights.float()).mean().item()
#     return accuracy*100
...
def main(args):
    ...
    test_iter = SentPairDataLoader(args.test_file,
                                   cfg.batch_size,
                                   tokenize,
                                   model_cfg.max_len,
                                   pipeline=pipeline)
    ...
    # optimizer = optim.optim4GPU(cfg, model) # by author
    optimizer = torch.optim.AdamW(
            params=model.parameters(),lr=cfg.lr) # by self
    
    trainer = train.Trainer(cfg, model, data_iter, test_iter, optimizer, evaluate_mlm, evaluate_sop, args.save_dir, get_device())
    ...
    def get_loss(model, batch, global_step): # make sure loss is tensor
        ...
        # print(logits_lm.transpose(1, 2).size(), masked_ids.size())
        loss_lm = (loss_lm*masked_weights.float()).mean()
        loss_sop = criterion2(logits_clsf, is_next) # for sentence classification
        return loss_lm, loss_sop

    trainer.train(get_loss, model_file=None, data_parallel=False) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_file', type=str, default='./data/wiki.test.tokens')
    ...
    parser.add_argument('--max_pred', type=int, default=75, help='max tokens of prediction')
