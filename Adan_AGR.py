...
class Adan(Optimizer):
    ...
    @torch.no_grad()
    def step(self, i, coefficient, device, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                #GC operation for Conv layers and FC layers with adapative learning of coefficient
                grad = p.grad
                grad_sum = torch.abs(grad).sum(dim=tuple(range(0,len(grad.size()))),keepdim=True)
                p.grad = grad - torch.mul(grad,torch.abs(grad)/grad_sum)
                ### 
                grads.append(p.grad)
                state = self.state[p]
                if len(state) == 0:
